...
public class UserAchiPointJDBCTemplate implements UserAchiPointDAO {
	...
	// UserAchiPoint userAchiPoint = jdbcTemplate.queryForObject(sqlComm, new
	// Object[]{id}, new UserAchiPointMapper());
	//
	// return userAchiPoint;
	// }
	//
	// public int getUserAchiPointCount_byUsername(String username){
	// String sqlComm = "SELECT COUNT(*) FROM user_achievement_point uap, user u
	// WHERE u.id_user = uap.user_id AND u.username = ?";
	// int userAchiPointCount = jdbcTemplate.queryForObject(sqlComm, Integer.class,
	// username);
	//
	...
	// String sqlComm = "SELECT *, COUNT( * ) AS 'count' FROM user_achievement_point
	// uap, user u "
	// + "WHERE u.id_user = uap.user_id "
	// + "GROUP BY u.username "
	// + "ORDER BY COUNT( * ) DESC ";
	// List<UserAchiPoint> userAchiPointRatingsList = jdbcTemplate.query(sqlComm,
	// new Object[]{}, new UserAchiPointMapper());
	//
	...
	// String sqlComm = "INSERT INTO user_achievement_point (user_id,
	// achievement_id) VALUES ('" + user_id + "','" + achievement_id + "')";
	// jdbcTeamplateObject.update(sqlComm);
	// }

	/*
	 * public int getUserAchiPointRatingPosition_byUsername(String username){ String
	 * sqlComm =
	 * "SELECT COUNT(*) FROM user_achievement_point uap, user u WHERE u.id_user = uap.user_id AND u.username = ?"
	 * ; int userAchiPointCount = jdbcTemplate.queryForObject(sqlComm,
